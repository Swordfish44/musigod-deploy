...
module.exports=withSentry(async function(req,res){
 ...
 try{const profile=await resolveProfile(actor,req.method==='POST'?req.body?.profile_id:req.query?.profile_id);if(!profile)return res.status(404).json({error:'Rights profile not found'})
  ...
  if(action==='review_ocr'){
   if(!actor.admin)return res.status(403).json({error:'Named administrative reviewer required'});const reviewer=String(b.reviewer_name||'').trim(),notes=String(b.notes||'').trim(),decision=String(b.decision||'').toUpperCase();if(reviewer.length<2||notes.length<12||!['APPROVED','REJECTED'].includes(decision))return res.status(400).json({error:'OCR decision, named reviewer, and substantive review notes are required'})
   const record=(await sb(`ocr_executions_v1?id=eq.${encodeURIComponent(b.ocr_execution_id)}&profile_id=eq.${profile.id}&limit=1`,{schema:SCHEMA}))?.[0];if(!record)return res.status(404).json({error:'OCR execution not found'});if(record.status!=='PENDING_REVIEW')return res.status(409).json({error:'OCR execution was already reviewed'})
   await sb(`ocr_executions_v1?id=eq.${record.id}`,{schema:SCHEMA,method:'PATCH',body:{status:decision,named_reviewer:reviewer,resolution_notes:notes,reviewed_at:new Date().toISOString()}})
   await audit(profile.id,actor,`statement_ocr.${decision.toLowerCase()}`,'ocr_execution',record.id,{source_file_id:record.source_file_id,page_number:record.page_number,confidence:record.confidence,reviewer});return res.status(200).json({ok:true,status:decision,ocr_execution_id:record.id})
  }
...
async function overview(profileId,admin){const q=(t,s='')=>sb(`${t}?profile_id=eq.${profileId}${s}`,{schema:SCHEMA});const [packages,jobs,files,exceptions,approvals,ocr,adapters]=await Promise.all([q('statement_packages_v1','&order=created_at.desc&limit=50'),q('ingestion_jobs_v1','&order=created_at.desc&limit=100'),q('source_files_v1','&order=created_at.desc&limit=100'),q('import_exceptions_v1','&review_status=in.(OPEN,IN_REVIEW,LEGAL_REVIEW)&order=created_at.asc&limit=200'),q('import_approvals_v1','&order=approved_at.desc&limit=50'),q('ocr_executions_v1','&order=created_at.desc&limit=200'),sb('adapter_versions_v1?approval_status=in.(TESTED,APPROVED)&order=adapter_key',{schema:SCHEMA})]);return {packages,jobs:admin?jobs:jobs.map(({lease_owner,last_error_safe,...x})=>x),files,exceptions,approvals,ocr,adapters,capacity:{maximum_declared_file_bytes:MAX,demonstrated_fixture_rows:1000000,ten_million_rows:'projected_not_yet_certified'},disclosure:'Reported statement data is not independently verified. Calculated variances are not money owed, claimed, settled, or recovered.'}}
