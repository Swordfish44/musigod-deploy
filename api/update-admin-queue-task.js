...
const ALLOWED_STATUSES = new Set(['OPEN','IN_PROGRESS','BLOCKED','COMPLETE','CANCELLED'])
...
module.exports = withSentry(async function handler(req, res) {
  ...
  const { task_id, status, assigned_to, task_body } = body
  ...
  try {
    ...
    if (task_body)   patch.task_body = task_body
    ...
    if (!res2.ok) throw new Error(`Supabase PATCH failed: ${res2.status} ${text}`)
    ...
    console.error('update-admin-queue-task error:', err)
    captureException(err, { route: 'update-admin-queue-task' })
    return res.status(500).json({ error: 'Could not update task' })
  }
}, 'update-admin-queue-task')
