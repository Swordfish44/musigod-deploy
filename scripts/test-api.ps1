...
$payloads = @{
    "submit-statement" = @{
        ...
        file_type = "MANUAL"
        notes = "Q1 2024 test ingestion"
        line_items = @(
            @{ song_title="Test Track 001"; isrc="USRC12400001"; royalty_type="PERFORMANCE"; gross_amount_usd=250; is_recovery=$true }
            @{ song_title="Test Track 002"; isrc="USRC12400002"; royalty_type="PERFORMANCE"; gross_amount_usd=180; is_recovery=$false }
        )
    }
}
...
$url = "$base/$Endpoint"
$body = $payloads[$Endpoint] | ConvertTo-Json -Depth 5
