...
function estimateRoyalties(results, artistName) {
  ...
  if (sx?.found !== true && discogs?.mainReleases > 3) {
    // Unregistered or unconfirmed (found === false or null — manual-check-only
    // mode when the authorized API adapter isn't enabled) with long catalog =
    // likely uncollected digital performance
