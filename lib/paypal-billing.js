...
const sandboxPlanIds = require('../generated/paypal-sandbox-plans.json')
...
// Env var wins. Outside production + live mode, fall back to the committed
// sandbox plan IDs so a Preview never depends on a write-only Vercel secret.
function sandboxFallbackAllowed() {
  return process.env.PAYPAL_ENV !== 'live' && process.env.VERCEL_ENV !== 'production'
}
...
function planIdFor(plan) {
  ...
  return sandboxFallbackAllowed() ? sandboxPlanIds[key] || null : null
}

function planForId(planId) {
  if (!planId) return null
  return Object.keys(PLAN_ENV).find(plan => planIdFor(plan) === planId) || null
}
...
async function createSubscription({ artistId, plan, email, siteUrl, fetchImpl = fetch }) {
  const normalizedPlan = String(plan || '').toLowerCase()
  const planId = planIdFor(normalizedPlan)
  if (!planId) throw new Error('PayPal plan is not configured')

  const baseUrl = (siteUrl || process.env.PUBLIC_SITE_URL || 'https://musigod.com').replace(/\/$/, '')
