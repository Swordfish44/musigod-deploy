...
// Strip MB release title to core album name for Discogs search
// "Judgement Day, Volume 3: Ascending" → "Judgement Day"
function coreTitle(title) {
  return (title || '').split(/[,:(]/)[0].trim();
}

// Writer-role strings used to detect creditable releases
const WRITER_ROLES = ['written-by', 'music by', 'lyrics by', 'words by', 'composed by',
  'written by', 'music', 'lyrics', 'words', 'songwriter'];

function hasWriterCredits(discogsReleaseData) {
  return (discogsReleaseData.extraartists || []).some(a =>
    WRITER_ROLES.some(role => a.role?.toLowerCase().includes(role))
  ) || (discogsReleaseData.tracklist || []).some(t =>
    (t.extraartists || []).some(a =>
      WRITER_ROLES.some(role => a.role?.toLowerCase().includes(role))
    )
  );
}

// Search for releases by artist + core title, returning up to `limit` sorted candidates.
// Title-matching results come first.
async function findReleases(artistName, releaseTitle, limit = 10) {
  await sleep(DISCOGS_RATE_MS);
  const core = coreTitle(releaseTitle);
  const q = `${artistName} ${core}`;
  const data = await discogsGet(`/database/search?q=${encodeURIComponent(q)}&type=release&per_page=${limit}`);
  const results = data.results || [];
  if (!results.length) return [];
  const norm = s => (s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  const coreNorm = norm(core);
  // Sort: results whose title contains the core album name first
  return [
    ...results.filter(r => norm(r.title).includes(coreNorm)),
    ...results.filter(r => !norm(r.title).includes(coreNorm)),
  ];
}

// Search for a specific release by artist + core title (single-result legacy wrapper)
async function findRelease(artistName, releaseTitle) {
  const results = await findReleases(artistName, releaseTitle, 10);
...
module.exports.findReleases      = findReleases;
