...
// Search for a specific release by artist + core title
async function findRelease(artistName, releaseTitle) {
  ...
  const data = await discogsGet(`/database/search?q=${encodeURIComponent(q)}&type=release&per_page=10`);
  const results = data.results || [];
  if (!results.length) return null;
  ...
  // Prefer result whose title contains the core album name
  const match = results.find(r => norm(r.title).includes(coreNorm));
