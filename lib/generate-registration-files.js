...
// SoundExchange partner-ready repertoire package. This is a controlled
// preparation format, not evidence of an official API submission.
function generateSoundExchangeCSV(tracks) {
  const rows = [['ISRC','Recording Title','Featured Artist','SRCO / Master Rights Holder','Label','Release Date','Duration (seconds)','Territory','Ownership Evidence Reference','Submission Note']];
  for (const track of tracks) {
    const title = track.trackTitle || track.track_title || '';
    const artist = track.artistName || track.artist_name || (track.artistCredits || []).join('; ');
    const isrcs = track.isrcs || [];
    for (const isrc of isrcs) rows.push([isrc,title,artist,track.master_rights_holder || '',track.label_name || '',track.release_date || track.releaseDate || '',track.trackDuration || track.track_duration || '',track.territory || 'US',track.ownership_evidence_reference || '','PARTNER-READY PACKAGE — EXTERNAL SUBMISSION REQUIRES SOUNDEXCHANGE AUTHORIZATION']);
  }
  return stringify(rows);
}
...
function generateGapsReport(tracks) {
  ...
  return {
    ...
    writersPresent: tracks.filter(t => t.writers.length > 0).length,
    // NOTE: writersPresent is NOT a readiness count. Writer names alone do not
    // constitute registration readiness. IPI, PRO affiliation, confirmed splits,
    // and destination-specific rules must all be satisfied.
    missingWriterTracks: noWriter.slice(0, 50).map(t => ({ releaseTitle: t.releaseTitle, trackTitle: t.trackTitle })),
  };
}

// ── EXPORT GUARD ─────────────────────────────────────────────────────────────
// Fail-closed guard. readinessMap: Map<trackTitle_lower, { decision, blockers }>
// Throws EXPORT_BLOCKED if any track is not READY or has no evaluated readiness.
function assertExportReady(tracks, readinessMap) {
  const nonReady = [];
  for (const track of tracks) {
    const key = (track.trackTitle || '').toLowerCase().trim();
    const r = readinessMap && readinessMap.get(key);
    if (!r || r.decision !== 'READY') {
      nonReady.push({
        trackTitle: track.trackTitle,
        decision:   r ? r.decision : 'NOT_EVALUATED',
        blockers:   r ? r.blockers : [{ code: 'NOT_EVALUATED', severity: 'BLOCKING',
          message: 'Readiness not evaluated for this track. Run /api/evaluate-readiness first.' }],
      });
    }
  }
  if (nonReady.length > 0) {
    const err = new Error(`Export blocked: ${nonReady.length} track(s) are not READY for registration`);
    err.code = 'EXPORT_BLOCKED';
    err.nonReady = nonReady;
    throw err;
  }
}
