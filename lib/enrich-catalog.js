...
async function getDiscogsWritersForTrack(artistName, releaseTitle, trackTitle) {
  ...
  if (!discogsCache.has(cacheKey)) {
    try {
      ...
      // Try up to 3 candidates; stop at first with writer credits.
      // Cap at 3 (not 5) to stay within the 300s Vercel budget:
      // worst case 30 albums × (1 search + 3 credits) × 1100ms + 102s MB ≈ 264s < 300s.
...
async function enrichArtistCatalog(artistName, options = {}) {
  ...
  // Time budget guard: Genius has no rate limit — cap total elapsed time so
  // Genius fallback calls don't push the function past the 300s Vercel limit.
  const BUDGET_MS = 240_000; // 240s leaves 60s buffer
  const startMs = Date.now();
  const withinBudget = () => (Date.now() - startMs) < BUDGET_MS;
  ...
  for (let i = 0; i < toProcess.length; i++) {
    if (!withinBudget()) {
      console.warn(`[enrich] Budget guard: stopping at release ${i}/${toProcess.length}, ${enrichedTracks.length} tracks collected`);
      break;
    }
      ...
      try {
        ...
        if (!workRels.length) {
          ...
          if (dw.length) {
            ...
            trackData.enrichmentError = 'No work in MB; no credits on Discogs; Genius skipped (budget)';
            console.warn(`[enrich] Genius skipped for "${t.trackTitle}" — budget exceeded`);
          }
          enrichedTracks.push(trackData);
          continue;
        }
          ...
          try {
            ...
            if (trackData.writers.length === 0) {
              ...
              if (dw.length) {
                ...
                trackData.enrichmentError = 'Work in MB but no writer credits; Genius skipped (budget)';
                console.warn(`[enrich] Genius skipped for "${t.trackTitle}" — budget exceeded`);
              }
            }
          } catch (workErr) {
