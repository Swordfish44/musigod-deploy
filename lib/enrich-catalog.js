...
async function getDiscogsWritersForTrack(artistName, releaseTitle, trackTitle) {
  ...
  if (!discogsCache.has(cacheKey)) {
    try {
      ...
      console.log(`[discogs] findRelease("${artistName}", "${releaseTitle}") → ${release ? release.id + ' ' + release.title : 'null'}`);
      if (release) {
        const credits = await getReleaseCredits(release.id);
        console.log(`[discogs] getReleaseCredits(${release.id}) → ${credits.tracks.length} tracks, ${credits.albumWriters.length} album writers`);
        discogsCache.set(cacheKey, credits);
      } else {
          ...
          try {
            ...
            const writerRels = allArtistRels.filter(r =>
              ['composer','lyricist','writer','music','lyrics',
               ...
               'performance','performing orchestra'].includes(r.type)
            );
