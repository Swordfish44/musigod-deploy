...
const { getTrackWriters } = require('./discogs');
    ...
    for (const medium of media) {
      ...
      for (const track of tracks) {
        ...
        try {
          ...
          if (!workRels.length) {
            // MB has no linked work — fall back to Discogs for writer credits
            const discogsWriters = await getTrackWriters(artistName, rg.title, trackData.trackTitle);
            if (discogsWriters.length > 0) {
              trackData.writers = discogsWriters.map(w => ({
                name:   w.name,
                mbid:   null,
                role:   w.role || 'writer',
                ipi:    null,
              ...
              console.log(`[enrich] Discogs fallback: ${discogsWriters.length} writers for "${trackData.trackTitle}"`);
            } else {
              trackData.enrichmentError = 'No work in MusicBrainz; no credits on Discogs';
            }
            enrichedTracks.push(trackData);
            continue;
          }
          ...
          // MB work exists but has no writer credits — try Discogs
          if (!trackData.enriched) {
            ...
            if (discogsWriters.length > 0) {
              ...
              trackData.enriched = true;
              console.log(`[enrich] Discogs fallback (work no credits): ${discogsWriters.length} writers for "${trackData.trackTitle}"`);
            } else {
              trackData.enrichmentError = 'Work in MusicBrainz but no writer credits; not on Discogs either';
            }
          }
