...
const RATE_LIMIT_MS = 1100; // MusicBrainz requires max 1 req/sec
...
async function getReleaseGroups(artistMBID) {
  ...
  while (offset < total) {
    const data = await mbGet(
      `/release-group?artist=${artistMBID}&type=album|single|ep&limit=${limit}&offset=${offset}&inc=releases`
    );
        ...
        for (const track of tracks) {
          ...
          if (recording.id) {
            try {
              ...
              for (const wr of workRels.slice(0, 3)) { // limit to 3 works per recording
                if (wr.work?.id) {
                  try {
                    const work = await getWorkDetails(wr.work.id);
                    trackData.iswc = trackData.iswc || work.iswc || null;

                    // Extract writer credits from work relations
                    ...
                    for (const wRel of writerRels) {
                      if (wRel.artist) {
                        trackData.writers.push({
                          ...
                          mbid: wRel.artist.id,
                          role: wRel.type,
                          ipi: null, // IPI not in MB — needs PRO lookup
                        });
                      }
                    }
                    trackData.enriched = true;
                  } catch (e) {
                    // Work detail fetch failed — non-fatal
                  }
                }
              }
            } catch (e) {
