...
//
// Discogs fallback runs after MB for tracks still missing writers.

const { findRelease, getReleaseCredits } = require('./discogs');
...
async function getDiscogsWritersForTrack(artistName, releaseTitle, trackTitle) {
  ...
  let credits = discogsCache.get(cacheKey);

  if (!credits) {
    try {
      const release = await findRelease(artistName, releaseTitle);
      if (release) {
        credits = await getReleaseCredits(release.id);
      } else {
        credits = null;
      }
    } catch (err) {
      console.warn(`[enrich] Discogs release fetch failed for "${releaseTitle}": ${err.message}`);
      credits = null;
    }
    discogsCache.set(cacheKey, credits); // cache null too to avoid re-fetching misses
  }
  ...
  if (!track) return credits.albumWriters || [];
  return track.writers.length > 0 ? track.writers : (credits.albumWriters || []);
}
      ...
      try {
        ...
        if (!workRels.length) {
          ...
          if (dw.length) {
            ...
            trackData.enrichmentError = 'No work in MB; no credits on Discogs';
          }
          enrichedTracks.push(trackData);
          continue;
        }
          ...
          try {
            ...
            const writerRels = (work.relations || []).filter(r =>
              ...
              ['composer','lyricist','writer','music','lyrics',
               'composer-lyricist','arranger','words'].includes(r.type)
            );
            ...
            // Work exists but no credits — try Discogs
            if (!trackData.enriched) {
              ...
              if (dw.length) {
                ...
                trackData.enrichmentError = 'Work in MB but no writer credits; not on Discogs either';
              }
            }
          } catch (workErr) {
