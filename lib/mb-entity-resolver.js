...
// Strips hyphens, dots, and spaces from an ISWC string (e.g. T-345246800-1 → T3452468001).
// Applied before corpus writes (dump transformer) AND before lookups so the conflict
// key (mb_work_id, iswc) is stable regardless of which source supplied the value.
...
async function resolveTrack(track) {
  ...
  if (track.iswc) {
    const normISWC = normalizeISWC(track.iswc);
    let rows;
    try {
      rows = await corpusLookupISWC(normISWC);
    } catch (e) {
    ...
    for (const row of rows) {
      addCandidate({
        ...
        confidence:          0.95,
        match_signals:       { iswc: normISWC, iswc_match: true },
      });
    }
  }
...
// Paginates automatically; returns all rows regardless of catalog size.
async function loadEnrichedTracks(artistName = null) {
  ...
  while (true) {
    let params = `select=id,artist_name,track_title,recording_mbid,isrcs,iswc,track_duration&limit=${PAGE_SIZE}&offset=${offset}`;
...
module.exports = { resolveTrack, resolveAllTracks, persistCandidates, loadEnrichedTracks, titleSimilarity, normalize, PAGE_SIZE };
