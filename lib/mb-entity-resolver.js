...
// Uses public-schema SECURITY DEFINER RPCs instead of Accept-Profile: mb_staging
// to avoid dependence on the PostgREST "Exposed schemas" dashboard setting.

async function mbRpc(fnName, params) {
  ...
  if (!res.ok) throw new Error(`mb-entity-resolver rpc/${fnName}: ${res.status} ${text.slice(0, 200)}`);
...
async function resolveTrack(track) {
  ...
  for (const isrc of (track.isrcs || [])) {
    ...
    let rows;
    try {
      rows = await mbRpc('fn_mb_lookup_isrc', { p_isrc: norm });
    } catch (e) {
  ...
  if (track.recording_mbid) {
    let rows;
    try {
      rows = await mbRpc('fn_mb_lookup_recording_mbid', { p_mbid: track.recording_mbid });
    } catch (e) {
  ...
  if (track.iswc) {
    let rows;
    try {
      rows = await mbRpc('fn_mb_lookup_iswc', { p_iswc: track.iswc });
    } catch (e) {
  ...
  if (!hasHighConfidenceMatch && track.track_title) {
    ...
    if (normTitle.length >= 3) {
      ...
      let rows = [];
      try {
        rows = await mbRpc('fn_mb_search_recordings', { p_title: track.track_title.slice(0, 30) });
      } catch (e) {
