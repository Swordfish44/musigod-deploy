...
const signing = require('./agreement-signing')
...
async function activateOrHoldForAgreement({ artistId, plan, provider, subscriptionId }) {
  ...
  try {
    ...
    // First time only (webhooks are redelivered): email the artist their signing link.
    if (artist.meta?.billing_status !== PAID_AWAITING_AGREEMENT) {
      await sendSigningEmail(artist).catch(e => console.warn('SIGNING_EMAIL_FAILED', { artist_id: artistId, error: e.message }))
    }
...
async function sendSigningEmail(artist, baseUrl) {
  const key = process.env.RESEND_API_KEY
  if (!key || !artist?.email) return false
  const url = signing.signingUrl(baseUrl || signing.siteBaseUrl(null), artist.id)
  const from = process.env.FROM_EMAIL || 'MusiGod <support@musigod.com>'
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;color:#111;line-height:1.55">
      <h2 style="margin-bottom:8px">Payment received. One last step.</h2>
      <p>Thank you. Your MusiGod payment is confirmed.</p>
      <p><strong>Sign your Publishing Administration Agreement to activate your membership.</strong> It takes about a minute.</p>
      <p style="margin:24px 0"><a href="${url}" style="display:inline-block;background:#e8262a;color:#fff;text-decoration:none;font-weight:700;padding:13px 20px;border-radius:6px">Sign Your Agreement</a></p>
      <p style="font-size:13px;color:#666">This link is personal to you and expires in 30 days. Questions? Reply to this email.</p>
    </div>`
  const res = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ from, to: artist.email, subject: 'Action required: sign your MusiGod agreement to activate', html }),
  })
