-- ============================================================
-- 07_Recording_Identity_Fix_FINAL.sql
-- MusiGod Graph — Final Production Recording Identity Migration
--
-- SOURCE: public._musigod_fn_backup_20260718 (retrieved 2026-07-19)
-- This file contains the COMPLETE production function body with
-- all changes applied. No operator paste required. Review diff
-- against pg_get_functiondef() before executing.
--
-- Changes applied to production backup body:
--   CHANGE-1  DECLARE: 5 recording identity variables added
--   CHANGE-2  Section 3 (recording node): single-tier ISRC lookup
--             replaced with three-tier lookup + conflict logging
--   Edge fix  has_recording replaces recorded_as (work→recording edge)
--   Edge fix  performed replaces performed_by; direction reversed
--             (from artist→recording, was recording→artist)
--   CHANGE-3  COMMENT ON FUNCTION updated
--
-- Verified production state (2026-07-19, project uykzkrnoetcldeuxzqyy):
--   musigod_catalog_track namespace rows      = 0  (safe to introduce)
--   recording nodes using that namespace      = 0 rows
--   planned fallback identity collisions      = 0 rows
--
-- Rollback:  07_Recording_Identity_Rollback_FINAL.sql
-- Tests:     07_Recording_Identity_Smoke_Test_FINAL.sql
-- Pre-apply: 07_Recording_Identity_Verification_FINAL.sql
-- ============================================================


-- ══════════════════════════════════════════════════════════════════════
-- STEP 0  Pre-apply verification (READ-ONLY — run first, stop if any fail)
-- ══════════════════════════════════════════════════════════════════════

SELECT
  proname                                             AS fn_name,
  pg_get_function_result(oid)                         AS return_type,
  (pg_get_function_result(oid) = 'jsonb')             AS assert_returns_jsonb,
  length(pg_get_functiondef(oid))                     AS live_body_length,
  (length(pg_get_functiondef(oid)) > 10000)           AS assert_body_substantial,
  (pg_get_functiondef(oid) LIKE '%graph_nodes_v1%')   AS uses_graph_nodes_v1,
  (pg_get_functiondef(oid) LIKE '%fn_open_investigation%') AS has_investigation_logic
FROM pg_proc
WHERE proname      = 'fn_sync_track_to_graph'
  AND pronamespace = 'public'::regnamespace;

-- STOP if assert_returns_jsonb = f
-- STOP if assert_body_substantial = f
-- STOP if uses_graph_nodes_v1 = f  (live function may differ from backup)
-- STOP if 0 rows returned (function not found)


-- ══════════════════════════════════════════════════════════════════════
-- STEP 1  Create identity conflict table (idempotent, run before STEP 2)
-- ══════════════════════════════════════════════════════════════════════
...
--
-- Full executable body — no placeholder paste required.
-- Source: public._musigod_fn_backup_20260718 + CHANGE-1/-2 + edge fixes.
-- Review diff against STEP 0 pg_get_functiondef output before executing.
-- ══════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.fn_sync_track_to_graph(uuid) IS
  ...
  'performed_by->performed, performed direction artist->recording. '
  'Recording identity corrected 2026-07-19: three-tier lookup '
  '(ISRC -> recording_mbid -> catalog_link fallback) replaces single-tier '
  'unnormalized ISRC lookup. Conflict logging: graph.recording_identity_conflicts.';
...
SELECT
  ...
  (length(pg_get_functiondef(oid)) > 15000)                            AS body_length_ok,
  (pg_get_functiondef(oid) LIKE '%v_norm_isrc%')                       AS has_norm_isrc_var,
  (pg_get_functiondef(oid) LIKE '%v_rec_node_isrc%')                   AS has_isrc_lookup_var,
  (pg_get_functiondef(oid) LIKE '%v_rec_node_fallback%')               AS has_fallback_var,
  (pg_get_functiondef(oid) LIKE '%graph_identifiers_v1%')              AS has_identifier_table,
  (pg_get_functiondef(oid) LIKE '%graph_catalog_links_v1%')            AS has_catalog_links,
  (pg_get_functiondef(oid) LIKE '%recording_identity_conflicts%')      AS has_conflict_log,
  (pg_get_functiondef(oid) LIKE '%musicbrainz_recording%')             AS has_mbid_ns,
  (pg_get_functiondef(oid) LIKE '%''has_recording''%')           AS has_has_recording_edge,
  (pg_get_functiondef(oid) LIKE '%''performed''%')               AS has_performed_edge,
  (pg_get_functiondef(oid) LIKE '%fn_open_investigation%')             AS has_investigations,
  (pg_get_functiondef(oid) LIKE '%ai_consent_v1%')                     AS has_ai_consent_check,
  (pg_get_functiondef(oid) LIKE '%graph_evidence_v1%')                 AS has_evidence,
  (pg_get_functiondef(oid) LIKE '%recorded_as%')                       AS stale_recorded_as,
  (pg_get_functiondef(oid) LIKE '%performed_by%')                      AS stale_performed_by
FROM pg_proc
WHERE proname      = 'fn_sync_track_to_graph'
  AND pronamespace = 'public'::regnamespace;

-- All booleans must be t EXCEPT stale_* which must be f.
-- If return_is_jsonb = f OR body_length_ok = f -> ROLLBACK immediately.
